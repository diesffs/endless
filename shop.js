...
export default class Shop {
  ...
  switchSubTab(subTab) {
    ...
    document
      .querySelector(`button[data-sub-tab="${subTab}"]`)
  ...
  updateShopUI(subTab) {
    ...
    const config =
      subTab === "gold-upgrades" ? UPGRADE_CONFIG : CRYSTAL_UPGRADE_CONFIG;
  ...
  createUpgradeButton(stat, config) {
    ...
    return `
      <button data-stat="${stat}">
        <span class="upgrade-name">${config.label} (Lvl ${
  ...
  buyUpgrade(stat) {
    ...
    this.hero.upgradeCosts[stat] += BASE_UPGRADE_COSTS[stat]; // Increase cost by base value

    this.updateShopUI(isCrystalUpgrade ? "crystal-upgrades" : "gold-upgrades");
    this.hero.displayStats();
    this.hero.recalculateFromAttributes();
